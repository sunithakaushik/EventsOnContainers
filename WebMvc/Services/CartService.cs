...
namespace WebMvc.Services
{
    public class CartService : ICartService
    {
        ...
        // this is where the token is retrieved from the identity server through IHttpContext 
        ...
        public async Task AddItemToCart(ApplicationUser user, CartItem product)
        {
            ...
            /*if (cart == null) //does not need this check now
            {
                cart = new Cart()
                {
                    BuyerId = user.Id,
            ...
            var basketItem = cart.Items
                ...
                .FirstOrDefault();
            if (basketItem == null) // if first time purchasing the item, then add the product to cart, else increase the qty by 1
            {
        ...
        public async Task<Cart> GetCart(ApplicationUser user)
        {
            ...
            //if no cart then a new one is created
        ...
        async Task<string> GetUserTokenAsync()
        {
            ...
            // the browser window in context from httpcontext - which has the access token 
