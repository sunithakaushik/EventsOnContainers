...
namespace WebMvc.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<int> CreateOrder(Order order)
        {
            ...
            var jsonString = response.Content.ReadAsStringAsync();  // just getting the orderId as the response here hence storing as a jsonstring

            jsonString.Wait();  // we need to wait for the response, and conversion as we are not using await, async - this is another way
            _logger.LogDebug("response " + jsonString);
            dynamic data = JObject.Parse(jsonString.Result);  // json is returning so we need to parse to get the id value from the json object

            //json key value pair, but json puts the value in camel case hence instead of OrderId (pascal case from controller api), we see orderId here  in the json object
